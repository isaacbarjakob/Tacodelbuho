...
let password=sessionStorage.getItem('taco_admin_password')||''; let content={}; let dirty=false;
...
function addEvent(e={},shouldMark=true){
  ...
  card.innerHTML=`<button class="remove-event" type="button">Ta bort</button><div class="grid"><label>Typ<input data-f="type" value="${esc(e.type||'EVENT')}"></label><label>Datum<input data-f="date" value="${esc(e.date||'')}"></label></div><label>Rubrik<input data-f="title" value="${esc(e.title||'')}"></label><label>Text<textarea data-f="text">${esc(e.text||'')}</textarea></label><div class="grid"><label>Tid<input data-f="time" value="${esc(e.time||'')}"></label><label>Notering<input data-f="note" value="${esc(e.note||'')}"></label></div>`;
  card.querySelector('.remove-event').onclick=()=>{card.remove();markDirty()};
...
async function upload(){const original=$('#image-file').files[0];if(!original)return setStatus('Välj en bild först.','error');setStatus('Anpassar och komprimerar bilden…','loading');$('#upload-btn').disabled=true;try{const file=await optimizeImage(original);const fd=new FormData();fd.append('file',file);setStatus('Laddar upp bilden…','loading');const d=await api('/api/admin/upload',{method:'POST',body:fd});$('#offer-image').value=d.path;updatePreview();dirty=true;setStatus('Bilden är klar. Tryck Spara & publicera.','warning')}catch(e){setStatus(e.message.includes('decode')?'Bilden kunde inte läsas. Spara den som JPG eller PNG och försök igen.':e.message,'error')}finally{$('#upload-btn').disabled=false}}
