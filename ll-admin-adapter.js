(() => {
  ...
  const showPreviewBanner = () => {
    if (!isEmbedded || document.getElementById("ll-admin-preview-banner")) return;
    const banner = document.createElement("div");
    banner.id = "ll-admin-preview-banner";
    banner.setAttribute("role", "status");
    banner.textContent = "LL ADMIN · PRIVAT TESTFÖRHANDSVISNING · LIVESIDAN ÄR ORÖRD";
  ...
  const applyEvents = (events) => {
    ...
    events.slice(0, 6).forEach((event) => {
      ...
      note.textContent = "Boka gärna bord";
  ...
  const applyImages = (images) => {
    ...
    images.forEach((item) => {
      ...
      if (label.includes("hero") || label.includes("start")) {
        const image = heroSlides[heroIndex++] || heroSlides[0];
        if (image) {
          image.src = url;
          image.alt = alt;
        }
      ...
      const image = foodImages[galleryIndex++] || foodImages[0];
      if (image) {
        image.src = url;
        image.alt = alt;
      }
  ...
  if (isEmbedded) {
    const announceReady = () => {
      window.parent.postMessage(
        { source: "taco-ll-adapter", type: READY_MESSAGE, version: 1 },
        LL_ADMIN_ORIGIN,
      );
    };
